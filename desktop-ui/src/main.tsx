...
const CaptureDialog=memo(function CaptureDialog({sources,selected,onSelect,onClose}:{sources:CaptureSource[];selected?:CaptureSource;onSelect(source:CaptureSource):void;onClose():void}){
  ...
  return <motion.div className="fixed inset-0 z-50 grid place-items-center bg-black/70 p-8 backdrop-blur-sm" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} transition={{duration:.18}} onMouseDown={onClose}>
    <motion.div className="w-full max-w-3xl overflow-hidden rounded-2xl border border-zinc-800/80 bg-[#121215]/95 shadow-2xl shadow-black/60 backdrop-blur-xl" initial={{opacity:0,scale:.975,y:10}} animate={{opacity:1,scale:1,y:0}} exit={{opacity:0,scale:.985,y:6}} transition={{duration:.22,ease:[.2,.8,.2,1]}} onMouseDown={event=>event.stopPropagation()}>
...
const Home=memo(function Home({onCreate,onJoin}:{onCreate(name:string):void;onJoin(name:string,code:string):void}){
  ...
  return <>
    <div className="relative isolate flex min-h-0 flex-1 items-center justify-center overflow-hidden bg-[#09090b] p-10">
      <div className="pointer-events-none absolute inset-0 opacity-20 [mask-image:radial-gradient(ellipse_60%_50%_at_50%_0%,#000_70%,transparent_100%)] bg-[linear-gradient(to_right,#1f1f2e_1px,transparent_1px),linear-gradient(to_bottom,#1f1f2e_1px,transparent_1px)] bg-[size:4rem_4rem]"/>
      <div className="pointer-events-none absolute -left-40 -top-40 h-96 w-96 rounded-full bg-purple-600/20 blur-[128px]"/>
      <div className="pointer-events-none absolute right-0 top-1/2 h-96 w-96 -translate-y-1/2 rounded-full bg-indigo-600/15 blur-[128px]"/>
...
function Room({session,onLeave}:{session:SessionState;onLeave():void}){
  ...
  return <div className="relative flex min-h-0 flex-1 overflow-hidden bg-[#09090b]">
...
function App(){
  ...
  const [splash,setSplash]=useState(()=>localStorage.getItem("lunira.desktop.welcomed.v1")!=="1");
  ...
  const finishSplash=useCallback(()=>{localStorage.setItem("lunira.desktop.welcomed.v1","1");setSplash(false);},[]);
