...
export function App(){
  ...
  const path=location.pathname;const watch=path.match(/^\/watch\/([A-Za-z0-9]+)\/?$/),queryRoom=new URLSearchParams(location.search).get("room")?.toUpperCase();if(watch)return <ViewerPage roomId={watch[1].toUpperCase()}/>;if(path==="/"&&queryRoom&&/^[A-Z2-9]{8}$/.test(queryRoom))return <ViewerPage roomId={queryRoom}/>;if(path==="/broadcast")return <BroadcasterPage/>;if(path==="/como-funciona")return <HowItWorksPage/>;return <HomePage/>;
}
