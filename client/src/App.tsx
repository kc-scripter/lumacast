...
export function App(){
  ...
  const path=location.pathname;const watch=path.match(/^\/watch\/([A-Za-z0-9]+)$/);if(watch)return <ViewerPage roomId={watch[1].toUpperCase()}/>;if(path==="/broadcast")return <BroadcasterPage/>;if(path==="/como-funciona")return <HowItWorksPage/>;return <HomePage/>;
}
