...
const videoBitrate=(quality:Quality,fps:FrameRate)=>quality==="1080p"?(fps===60?6500:4200):quality==="720p"?(fps===60?3800:2200):(fps===60?5200:3200);
const captureConstraints=(quality:Quality,settings:MediaTrackSettings):ExtendedMediaTrackConstraints=>{
  ...
  return{width:{ideal:size.width,max:size.width},height:{ideal:size.height,max:size.height},frameRate:{ideal:60,max:60}};
};
...
export function useCollaborativeRoom(owner:boolean,requestedRoomId?:string){
  ...
  useEffect(()=>()=>{if(fallbackTimerRef.current){clearTimeout(fallbackTimerRef.current);fallbackTimerRef.current=null;}subscriberRef.current?.remoteUsers.forEach(user=>user.audioTrack?.stop());void subscriberRef.current?.leave().catch(()=>undefined);void screenClientRef.current?.leave().catch(()=>undefined);screenTracksRef.current.forEach(track=>{try{track.close();}catch{}});screenTracksRef.current=[];screenLivekitTracksRef.current.forEach(track=>{try{track.stop();}catch{}});screenLivekitTracksRef.current=[];streamRef.current?.getTracks().forEach(track=>{try{track.stop();}catch{}});streamRef.current=null;cameraRef.current?.stop();cameraRef.current=null;const room=livekitRef.current;livekitRef.current=null;if(room)void room.disconnect(false);livekitAudioRef.current.forEach(element=>{try{element.pause();element.srcObject=null;element.remove();}catch{}});livekitAudioRef.current.clear();if(videoRef.current)videoRef.current.srcObject=null;},[]);
