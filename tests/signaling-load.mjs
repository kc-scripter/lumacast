...
const connect=()=>new Promise((resolve,reject)=>{const started=performance.now(),socket=io(url,{forceNew:true,transports:["websocket"],reconnection:false,timeout:5000,extraHeaders:{Origin:origin}});socket.once("connect",()=>resolve({socket,ms:performance.now()-started}));socket.once("connect_error",reject);});
...
try{
  for(let i=0;i<roomCount;i++){const {socket,ms}=await connect();sockets.push(socket);latencies.push(ms);const created=await ack(socket,"create-room",{displayName:`Load Host ${i+1}`});if(!created.ok)throw new Error(created.error||"Falha ao criar sala");rooms.push({roomId:created.roomId,inviteToken:created.inviteToken});}
  const results=await Promise.all(Array.from({length:userCount},async(_,index)=>{const started=performance.now();try{const {socket}=await connect();sockets.push(socket);const target=rooms[index%rooms.length];const joined=await ack(socket,"join-room",{roomId:target.roomId,inviteToken:target.inviteToken,displayName:`Load User ${index+1}`});const ms=performance.now()-started;latencies.push(ms);return{ok:!!joined.ok,error:joined.error||null};}catch(error){return{ok:false,error:String(error)};}}));
