...
export function registerSignaling(io) {
    ...
    io.on("connection", socket => {
        socket.on("create-room", async (ack) => { const existing = rooms.findByBroadcaster(socket.id); if (existing) {
            try {
                return safeAck(ack, { ok: true, roomId: existing.id, broadcasterToken: existing.token, dailyUrl: existing.dailyRoomUrl, dailyToken: await createDailyToken(existing.dailyRoomName, "broadcaster", socket.id), live: existing.live, viewers: existing.viewers.size });
            }
            catch (error) {
            ...
            const roomId = rooms.newId(), daily = await createDailyRoom(roomId), room = rooms.create(roomId, socket.id, daily.name, daily.url), dailyToken = await createDailyToken(room.dailyRoomName, "broadcaster", socket.id);
            socket.join(room.id);
            safeAck(ack, { ok: true, roomId: room.id, broadcasterToken: room.token, dailyUrl: room.dailyRoomUrl, dailyToken, viewers: 0 });
        }
        catch (error) {
            console.error("Daily room creation error", error);
            safeAck(ack, { ok: false, error: "Não foi possível criar a sala de transmissão." });
        } });
        socket.on("reclaim-room", async (input, ack) => { if (!allowed(socket))
            ...
            const dailyToken = await createDailyToken(room.dailyRoomName, "broadcaster", socket.id);
            socket.join(room.id);
            safeAck(ack, { ok: true, roomId: room.id, dailyUrl: room.dailyRoomUrl, dailyToken, live: room.live, viewers: room.viewers.size });
            io.to([...room.viewers]).emit("room-state", { live: room.live, count: room.viewers.size });
        }
        catch (error) {
            console.error("Daily token error", error);
            safeAck(ack, { ok: false, error: "Não foi possível retomar a transmissão." });
        } });
        socket.on("join-room", async (input, ack) => { if (!allowed(socket))
            ...
            const dailyToken = await createDailyToken(room.dailyRoomName, "viewer", socket.id), previous = rooms.findByViewer(socket.id);
            ...
            socket.join(id);
            safeAck(ack, { ok: true, live: room.live, dailyUrl: room.dailyRoomUrl, dailyToken });
            io.to(room.broadcasterId).emit("viewer-count", { count: room.viewers.size });
            io.to([...room.viewers]).emit("room-state", { live: room.live, count: room.viewers.size });
        }
        catch (error) {
            console.error("Daily viewer token error", error);
        ...
        socket.on("leave-room", (input) => { const id = input?.roomId; if (!validRoomId(id))
            ...
            return; room.viewers.delete(socket.id); socket.leave(id); io.to(room.broadcasterId).emit("viewer-count", { count: room.viewers.size }); io.to([...room.viewers]).emit("room-state", { live: room.live, count: room.viewers.size }); });
        ...
        socket.on("disconnect", () => { const broadcast = rooms.findByBroadcaster(socket.id); if (broadcast) {
            ...
            rooms.scheduleBroadcasterRemoval(broadcast.id, room => { io.to([...room.viewers]).emit("broadcast-ended"); void deleteDailyRoom(room.dailyRoomName); });
