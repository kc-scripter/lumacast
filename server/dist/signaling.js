...
import { newSecret, RoomStore, validRoomId } from "./rooms.js";
...
const state = (room) => ({ live: room.live, count: room.participants.size, activeScreenSharerId: room.activeScreenSharerId, activeScreenUid: room.activeScreenUid, screenProvider: room.screenProvider, livekitActive: livekitActive(room) });
export function registerSignaling(io) {
    ...
    io.on("connection", socket => {
        ...
        socket.on("create-room", (ack) => { if (!allowed(socket, 5, 60_000))
            return reply(ack, { ok: false, error: "Muitas tentativas. Aguarde um minuto." }); try {
            const existing = rooms.findByOwner(socket.id), room = existing ?? rooms.create(rooms.newId(), socket.id, createAgoraUid());
            socket.join(room.id);
            reply(ack, { ok: true, roomId: room.id, ownerToken: room.ownerToken, broadcasterToken: room.ownerToken, ...createAgoraCredentials(room.id, room.ownerUid, "viewer"), ...state(room) });
        }
        catch (error) {
        ...
        socket.on("join-room", (input, ack) => { if (!allowed(socket))
            ...
            if (!participant) {
                if (old) {
                    ...
                    participant = { socketId: socket.id, agoraUid: uid, token: newSecret(), livekitActive: false };
                }
                room.participants.set(socket.id, participant);
            }
            socket.join(room.id);
            reply(ack, { ok: true, participantToken: participant.token, ...createAgoraCredentials(room.id, participant.agoraUid, "viewer"), ...state(room) });
            announce(room);
        }
        catch (error) {
