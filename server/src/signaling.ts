...
import { configurationError,runtimeEnvironmentStatus } from "./environment.js";
...
const rtcReady=(ack:Ack)=>{
  if(runtimeEnvironmentStatus().ready)return true;
  reply(ack,configurationError());
  return false;
};
...
export async function registerSignaling(io:Server){const rooms=new RoomStore(createRoomPersistenceFromEnv());await rooms.hydrate();
  ...
  io.on("connection",socket=>{
    ...
    socket.on("request-screen-share",(input:unknown,ack:Ack)=>{if(!allowed(socket))return reply(ack,{ok:false,error:"Muitas tentativas."});if(!rtcReady(ack))return;const room=member(input);if(!room)return reply(ack,{ok:false,error:"Participante não autorizado."});if(room.activeScreenSharerId&&room.activeScreenSharerId!==socket.id)return reply(ack,{ok:false,error:"Outra pessoa já está compartilhando a tela."});try{if(!room.activeScreenSharerId){room.activeScreenSharerId=socket.id;let uid:number;do{uid=createAgoraUid();}while(uid===room.ownerUid||[...room.participants.values()].some(p=>p.agoraUid===uid));room.activeScreenUid=uid;room.screenProvider="agora";}reply(ack,{ok:true,...createAgoraCredentials(room.id,room.activeScreenUid!,"broadcaster"),...state(room)});announce(room);}catch(error){console.error("Screen lock error",error);reply(ack,{ok:false,error:"Não foi possível preparar a tela."});}});
    ...
    socket.on("renew-agora-token",(input:unknown,ack:Ack)=>{if(!allowed(socket,30,60_000,"credentials"))return reply(ack,{ok:false,error:"Muitas tentativas."});if(!rtcReady(ack))return;const room=member(input);if(!room)return reply(ack,{ok:false,error:"Participante não autorizado."});try{const screen=room.activeScreenSharerId===socket.id&&(input as {screen?:boolean})?.screen===true,uid=screen?room.activeScreenUid:rooms.getUid(room,socket.id);if(uid==null)return reply(ack,{ok:false,error:"UID inválido."});reply(ack,{ok:true,agoraToken:createAgoraCredentials(room.id,uid,screen?"broadcaster":"viewer").agoraToken});}catch(error){console.error("Agora renewal error",error);reply(ack,{ok:false,error:"Não foi possível renovar a conexão."});}});
    socket.on("get-livekit-token",async(input:unknown,ack:Ack)=>{if(!allowed(socket,30,60_000,"credentials"))return reply(ack,{ok:false,error:"Muitas tentativas."});if(!rtcReady(ack))return;const room=member(input);if(!room)return reply(ack,{ok:false,error:"Participante não autorizado."});try{const screen=room.activeScreenSharerId===socket.id,token=await createLiveKitToken(room.id,socket.id,screen);reply(ack,{ok:true,livekitUrl:process.env.LIVEKIT_URL,livekitToken:token});}catch(error){console.error("LiveKit token error",error);reply(ack,{ok:false,error:"Não foi possível conectar ao LiveKit."});}});
