...
export class RoomStore{
  ...
  async hydrate(){
    ...
    try{
      ...
      for(const item of saved){
        ...
        const room:Room={id:item.id,ownerId:item.ownerId,ownerName,ownerUid:item.ownerUid,ownerToken:null,ownerTokenHash,ownerLivekitActive:false,participants,activeScreenSharerId:null,activeScreenUid:null,screenProvider:"agora",live:false};
  ...
  private snapshot(room:Room):PersistedRoom{return{id:room.id,ownerId:room.ownerId,ownerName:room.ownerName,ownerUid:room.ownerUid,ownerTokenHash:room.ownerTokenHash,participants:[...room.participants.values()].map(({socketId,displayName,agoraUid,tokenHash})=>({socketId,displayName,agoraUid,tokenHash}))};}
